...
MongoDB.prototype.log = function(level, msg, opt_meta, cb) {
  ...
  process.nextTick(()=>{
    ...
    let errorInfo = opt_meta;
    let userId = errorInfo.userId || errorInfo.accountId || '';
    entry.type = errorInfo.type || 'server-error';
    entry.data = {
      userId: userId,
      postId: errorInfo.postId || (errorInfo.query && errorInfo.query.postId) || (errorInfo.params && errorInfo.params.postId) || undefined,
      url: errorInfo.url,
      method: errorInfo.method,
      postData: errorInfo.post || null,
    };
    ...
    entry.message = errorInfo.error && errorInfo.error.message || errorInfo.message || msg || '';
    entry.stack = errorInfo.error && errorInfo.error.stack || errorInfo.stack || 'No stack found';
